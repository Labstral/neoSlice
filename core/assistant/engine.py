...
_SYSTEM_PROMPT = (
    ...
    "- MODIFIER (quand l'utilisateur dit modifier/changer/corriger/mettre a jour/renommer) : "
    "update_product (nom* pour IDENTIFIER + le(s) champ(s) a changer : prix, grams, duree_h, "
    "notes, new_name) ; update_client (nom* + email/tel/adresse/ville/cp/pays/societe/notes/"
    "new_name) ; update_spool (material* + color pour identifier + remaining_g/price/brand/"
    "new_color) ; set_order_status (number* + status parmi : a faire / en impression / "
    "termine / livre / annule). Ex. 'change le prix de la figurine a 20' -> "
    ...
    "demande le client et les articles avec leurs prix, sans marqueur. S'il dit 'supprime un "
    "client' / 'supprime une bobine' / 'modifie un article' SANS preciser LEQUEL (article "
    "indefini) -> reponds 'Lequel ?' / 'Laquelle ?' sans marqueur, ne choisis JAMAIS un "
    "element au hasard dans la liste. Et 'supprime tout' / 'vide mon stock' / 'efface toutes "
    "mes donnees' -> refuse (pas de suppression en masse), sans marqueur. Ce n'est QUE lorsque "
    "tu as toutes les infos requises (ex. 'ajoute 1 kg de PLA rouge', 'supprime la bobine PLA "
    "noir') que tu emets le marqueur. Ne remplis JAMAIS un champ requis avec un placeholder "
    "<...>, une valeur inventee, ou une donnee piochee dans le contexte.\n"
...
_GUARD = (
    ...
    "JAMAIS 'PLA 1 kg') ; meme si la matiere/le prix sont donnes mais PAS le poids (ex. 'j'ai "
    "achete 2 bobines de PLA a 20 CHF') -> DEMANDE le poids par bobine, n'invente AUCUN poids "
    "(ni 250 g, ni 1 kg) ; 'cree un client' sans nom -> DEMANDE le nom (n'invente NI nom NI "
    ...
    "devis/commande) : s'il dit juste 'supprime une bobine' ou 'supprime un client' (article "
    "indefini, sans nom precis) -> DEMANDE lequel, ne choisis JAMAIS au hasard.\n"
    "- INTERDIT : suppression EN MASSE. 'supprime tout', 'tous/toutes mes X', 'efface toutes "
    "mes donnees', 'vide mon stock', 'efface tout' -> REFUSE, n'emets AUCUN marqueur, dis que "
    "tu ne fais pas de suppression massive (trop risque) et propose de supprimer UN element "
    "precis a la fois (ou de le faire dans l'Espace Pro). N'emets JAMAIS plusieurs marqueurs "
    "dans une seule reponse.\n"
    "- ENLEVER (2 sens) : 'enleve / retire LA bobine <X>' (l'objet ENTIER, SANS nombre de "
    "grammes) = delete_spool (supprimer la bobine). 'enleve / retire <N> g de <X>' (AVEC des "
    "grammes) = consume_spool (deduire du stock). Le nombre de grammes fait la difference.\n"
    "- STATUT : un enonce declaratif 'la commande <N> est terminee / en impression / livree / "
    "annulee' = mise a jour du statut -> emets set_order_status (pas seulement une phrase).\n"
    "- JAMAIS de fausse confirmation : ne dis JAMAIS 'c'est fait / cree / ajoute / supprime / "
    "modifie / enregistre avec succes' si tu n'as PAS emis le marqueur [[ACTION: …]] "
    "correspondant dans CE meme message. AGIR = emettre le marqueur ; sans marqueur, RIEN ne "
    "se passe et c'est neoSlice (pas toi) qui confirme le resultat reel. Donc : soit tu as "
    "toutes les infos et tu emets le marqueur, soit il en manque et tu poses une question — "
    "mais tu n'annonces jamais une action que tu n'as pas reellement declenchee.\n"
