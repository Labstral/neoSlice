"""Aperçu autonome de l'effet « Strands » (React Bits) porté en OpenGL natif Qt.

Le cœur de l'effet React Bits est un shader GLSL (indépendant de React/ogl). On le
réexécute ici dans un QOpenGLWidget — aucune dépendance web, aucun moteur Chromium.

Lancer :  python tools/strands_preview.py
(Si la fenêtre est noire : regarder la console pour d'éventuelles erreurs de shader.)

Une fois le rendu validé visuellement, la classe StrandsWidget sera déplacée dans
ui/components/ et posée en overlay dans le coin du viewer 3D.
"""
...
# Shader principal — les rubans lumineux (porté de React Bits, #version 300 es -> 330 core)
...
# Post-traitement — boule de verre réfractive
...
class StrandsWidget(QOpenGLWidget):
    """Effet Strands + boule de verre, rendu en OpenGL natif Qt."""

    def __init__(self, parent=None, colors=("#F97316", "#7C3AED", "#06B6D4"),
                 ...
                 dispersion=1.0, glassSize=0.57, bg=(0.055, 0.055, 0.08, 1.0)):
    ...
    def initializeGL(self):
        ...
        self._prog = self._make_program(FRAG)
        self._glass = self._make_program(GLASS_FRAG)
    ...
    def _make_program(self, frag):
        ...
        if not prog.addShaderFromSourceCode(QOpenGLShader.Vertex, VERT):
            print("VERT compile error:", prog.log())
        if not prog.addShaderFromSourceCode(QOpenGLShader.Fragment, frag):
            print("FRAG compile error:", prog.log())
        if not prog.link():
            print("LINK error:", prog.log())
        return prog
    ...
    def _u(self, prog, name, *vals):
        """IMPORTANT : dans cette version de PySide6, QOpenGLShaderProgram.setUniformValue
        est silencieusement inopérant → on passe par les fonctions GL brutes (glUniform*)."""
    ...
    def paintGL(self):
        ...
        if self.p["glass"] and self._fbo is not None:
            # Passe 1 : rubans -> texture
            self._fbo.bind()
            f.glClearColor(0.0, 0.0, 0.0, 0.0)
            f.glClear(GL_COLOR_BUFFER_BIT)
            self._prog.bind(); self._set_uniforms(self._prog); self._prog.release()
            self._draw(self._prog)
            self._fbo.release()
            f.glBindFramebuffer(GL_FRAMEBUFFER, self.defaultFramebufferObject())
            # Passe 2 : boule de verre
...
def main():
    ...
    win = QWidget(); win.setWindowTitle("neoSlice — aperçu effet Strands (test)")
    win.resize(760, 520); win.setStyleSheet("background:#0E0E14;")
    lay = QVBoxLayout(win); lay.setContentsMargins(0, 0, 0, 0)
    lay.addWidget(StrandsWidget())
    win.show()
    sys.exit(app.exec())
