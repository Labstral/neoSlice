...
class ParamsPreview(QWidget):
    ...
    def refresh_theme(self):
        ...
        for sec in self._sections.values():
            if hasattr(sec, "_btn"):
                ...
                """)
            if hasattr(sec, "_content"):
                sec._content.setStyleSheet(f"background: {pal['BG_PANEL']}")
            if hasattr(sec, "_update_header"):
                sec._update_header()
        self.update()
    ...
    def _render_sections(self, c: PrintConfig, a: AnalysisReport | None):
        ...
        s.add_row(_("preview.lbl_seams"),       c.seam_position.upper())
        s.add_row(_("preview.lbl_ironing"),     c.ironing_type.upper())
        if c.ironing_type != "no ironing":
            s.add_row(_("preview.lbl_iron_spd"),f"{c.ironing_speed} mm/s")
            s.add_row(_("preview.lbl_iron_flow"),f"{c.ironing_flow} %")
        s.add_row(_("preview.lbl_elef_foot"),   f"{c.elefant_foot_compensation} mm")
        ...
        if has_mat or has_support or has_mode:
            ...
            if has_support:
                label = _("preview.support_tree") if "tree" in c.support_type else _("preview.support_normal")
                s.add_row(_("preview.lbl_supports"),  label)
                s.add_row(_("preview.lbl_supp_thr"),  f"{c.support_threshold_angle:.0f}°")
                s.add_row(_("preview.lbl_build_only"),
            ...
            if "flush_into_infill" in fields and c.flush_into_infill:
                s.add_row(_("preview.lbl_flush_ams"), _("preview.lbl_infill"))
        ...
        if a and a.volume_cm3 > 0:
            ...
            s.add_row(_("preview.lbl_vol"),          f"{a.volume_cm3:.1f} cm³")
