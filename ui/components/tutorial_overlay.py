...
_STEPS: list[_Step] = [
    _Step(
        ...
        "neoSlice analyse votre fichier STL et génère automatiquement "
    ...
    _Step(
        "drop",
        "② Import STL",
        "Glissez votre <b>fichier STL</b> dans cette zone, "
...
class TutorialOverlay(QDialog):
    ...
    def _body_html(self) -> str:
        return _STEPS[self._idx].body.replace("\n", "<br>")
    ...
    def _paint_card(self, painter: QPainter):
        ...
        bd.setDefaultStyleSheet(
            f"body {{ color:{_BODY_COLOR}; }} b {{ color:{TEXT_PRIMARY}; font-weight:bold; }}"
        )
        bd.setHtml(
            f"<body style='font-family:Segoe UI;font-size:9pt;color:{_BODY_COLOR};'>"
            f"{self._body_html()}</body>"
    ...
    def _paint_btn(self, p: QPainter, rect: QRect, text: str,
        ...
        if variant == "primary":
            p.setBrush(QColor(ACCENT_BRIGHT if hovered else ACCENT))
            p.drawRoundedRect(rf, 4.0, 4.0)
            p.setPen(QColor("#020408"))
            p.setFont(QFont("Segoe UI", 9, QFont.Bold))
        elif variant == "outline":
            p.setBrush(QColor(30, 144, 255, 31) if hovered else Qt.NoBrush)
            p.drawRoundedRect(rf, 4.0, 4.0)
            ...
            p.setBrush(Qt.NoBrush)
            p.setPen(QPen(QColor(TEXT_SECONDARY if hovered else "#1A3550"), 1))
            p.drawRoundedRect(rf.adjusted(.5, .5, -.5, -.5), 4.0, 4.0)
            p.setPen(QColor(TEXT_PRIMARY if hovered else TEXT_SECONDARY))
