...
def optimize_orientation(
    ...
    for label, up_direction in candidates:
        ...
        stability_r = _fast_stability(r_verts, r_verts_z, r_com, r_com_z, height_mm)
...
def _fast_stability(
    ...
    # Pénalité : empreinte vs boîte englobante (extents du mesh roté)
    bb_xy = float(r_verts[:, 0].max() - r_verts[:, 0].min()) * float(r_verts[:, 1].max() - r_verts[:, 1].min())
    if bb_xy > 1e-6:
