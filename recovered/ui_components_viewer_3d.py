...
class Viewer3D(QWidget):
    ...
    def _add_build_plate(self, mesh=None) -> None:
        ...
        try:
            ...
            # Aligner half sur un multiple entier de spacing → carreaux complets
            half = np.ceil((plate_size / 2.0) / spacing) * spacing
            plate_size = half * 2.0
    ...
    def load_mesh(self, mesh) -> None:
        ...
        try:
            ...
            try:
                self._plotter.add_mesh(pv_mesh, color=_mesh_color, name="main_mesh")
        ...
        try:
            ...
            _pv_b = [_b[0][0]-_pad, _b[1][0]+_pad, _b[0][1]-_pad, _b[1][1]+_pad,
                     _b[0][2]-_pad*0.2, _b[1][2]+_pad*0.5]
            try:
